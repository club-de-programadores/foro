...
class ChatterController extends Controller
{
   ...
   //  $buscar = Discussion::where('slug','like',"%$slug%")->paginate(5);
//$slug = $request->get('slug');

        $pagination_results = config('chatter.paginate.num_of_results');
        $slug2 = $request->get('slug');
        $discussions = Discussion::where('slug','like',"%$slug2%")->with('user')->with('post')->with('postsCount')->with('category')->orderBy('created_at', 'DESC')->paginate($pagination_results);
        if (isset($slug)) {
